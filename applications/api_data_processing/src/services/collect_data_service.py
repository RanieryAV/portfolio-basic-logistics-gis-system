        ...
        try:
            ...
            if not geojson_path:
                raise FileNotFoundError(
                    f"GeoJSON file not found. Searched in: {possible_paths}"
                )
    ...
    def execute(self, batch_size: int = 100):
        ...
        try:
            ...
            possible_paths = [
                ...
                os.path.join(
                    local_root, "shared", "utils", "datasets", "bairros.geojson"
                ),  # Local execution
                os.path.join(
                    local_root, "shared", "utils", "datasets", "bairros.geojson.json"
                ),  # Local execution (Fallback)
            ]
            ...
            logger.info(
                f"Successfully processed {total_processed} neighborhoods in batches of {batch_size}."
            )
            return {
                "status": "success",
        ...
        try:
            ...
            possible_paths = [
                ...
                os.path.join(
                    ...
                    "utils",
                    "datasets",
                    "arruamento-al-osm.geojson.json",
                ),
                os.path.join(
                    local_root,
                    "shared",
            ...
            logger.info(
                f"Successfully processed {total_processed} streets in batches of {batch_size}."
            )
