...
class SavePostalAgenciesService:
    @staticmethod
    def upsert_postal_agencies_spark_df_to_db(spark_df, batch_size: int = 1):
        expected_cols = {
            "name",
            "address",
            "city",
        ...
        cleaned = spark_df.withColumn(
            "location_no_wrapper",
            F.regexp_replace(F.col("location"), r"^\s*(SRID=4326;)?POINT\s*\(", ""),
        ).withColumn(
            "location_no_wrapper",
            F.regexp_replace(F.col("location_no_wrapper"), r"\)\s*$", ""),
        )

        cleaned = cleaned.withColumn(
            "coords", F.split(F.col("location_no_wrapper"), r"\s+")
        )

        valid_df = cleaned.filter(F.size("coords") == 2)

        invalid_count = cleaned.filter(F.size("coords") != 2).count()

        if invalid_count > 0:
            logger.warning(f"Filtered out {invalid_count} invalid POINT geometries.")
        ...
        if "latitude" in valid_df.columns:
            valid_df = valid_df.withColumn("latitude", F.col("latitude").cast("double"))
        ...
        updatable_cols = [
            c.name for c in PostalAgencies.__table__.columns if c.name != "primary_key"
        ]
        ...
        try:
            ...
            if batch_size == 1:
                for row in iterator:
                    ...
                    try:
                        stmt = insert(PostalAgencies).values(db_row)
                        ...
                        stmt = stmt.on_conflict_do_update(
                            constraint="unique_name_zip_code", set_=update_dict
                        )
            ...
            ###################################################################
                ...
                for row in iterator:
                    ...
                    if len(batch) >= batch_size:
                        try:
                            stmt = insert(PostalAgencies).values(batch)
