...
class SparkSessionInitializer:
    ...
    def init_spark_session(spark_session_name):
        ...
        spark_master_url = os.getenv(
            "SPARK_MASTER_URL", f"spark://spark-master:{spark_master_rpc_port}"
        )
        ...
        spark_cores_max = os.getenv(
            "SPARK_CORES_MAX", "4"
        )  # total cores allowed for this app
        spark_executor_cores = os.getenv(
        ...
        spark_sql_debug_max_to_string_fields = os.getenv(
            "SPARK_SQL_DEBUG_MAX_TO_STRING_FIELDS", "10000"
        )

        # NEW: read RPC max-size in MB (user must give number in MB)
        spark_rpc_message_maxsize_mb = (
            2046  # default to 2046 MB (just under Spark's hard limit of 2047 MB)
        )
        try:
            if (
                ...
                logger.warning(
                    "SPARK_RPC_MESSAGE_MAXSIZE not set or invalid. Using Spark default of 128 MB."
                )
        ...
        if spark_rpc_message_maxsize_mb is not None:
            if spark_rpc_message_maxsize_mb > 2046:
                logger.warning(
                    "Requested SPARK_RPC_MESSAGE_MAXSIZE=%s > 2046 MB. Clamping to 2046 MB.",
                    spark_rpc_message_maxsize_mb,
                )
        ...
        spark = (
            ...
            .config(
                "spark.executorEnv.SPARK_LOCAL_DIRS", "/app/processed_output/spark_tmp"
            )
            .config(
                "spark.executor.extraJavaOptions",
                "-Djava.io.tmpdir=/app/processed_output/spark_tmp",
            )
